...
**Frontend pattern:** treat `page * pageSize >= total` as "no more pages." On the admin review list, `averageRating` is populated when filtering by a doctor and is otherwise `null`.
...
Each appointment response includes `hasReview`, allowing the patient UI to show the review action once and replace it with a submitted state afterwards.
...
- `GET /api/doctors/:id/reviews?page=&pageSize=` — public reviews for an active doctor, including `averageRating`.
- `GET /api/admin/reviews?doctorId=&userId=&page=&pageSize=` — admin-only moderation list.
...
- Docker deployments persist this directory in the `doctor-uploads` named volume
...
```tsx
<a
  href={`${API}/api/auth/oauth/start?provider=google&callbackURL=${encodeURIComponent('/dashboard')}`}
>
  Continue with Google
</a>
```
